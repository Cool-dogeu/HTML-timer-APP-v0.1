...
// Serial Display Manager class for MLED displays
class SerialDisplayManager {
  ...
  async connect() {
    try {
      ...
      console.error("Display connection failed:", error);
      this.onConnectionChange?.(false);
      return false;
    }
  }
  ...
  async disconnect() {
    try {
      ...
      console.error("Display disconnect error:", error);
    }
  }
  ...
  async sendCommand(command) {
    ...
    try {
      ...
      console.debug("Display command sent:", command);
    } catch (error) {
      console.error("Display send error:", error);
      throw error;
    }
  }

  // MLED Protocol Commands
  async setBrightness(level) {
    // Level 1=low, 2=medium, 3=high
    const command = `B${level}`;
  ...
  async sendText(zone, text, color = "green") {
    // MLED protocol: Z{zone}{color_code}{text}
    const colorCodes = {
      green: "G",
      red: "R",
      yellow: "Y",
    ...
    const colorCode = colorCodes[color] || "G";
    const command = `Z${zone}${colorCode}${text}`;
    await this.sendCommand(command);
  }
...
createApp({
  ...
  mounted() {
    ...
    // Initialize display serial manager
    this.displaySerialManager = new SerialDisplayManager();
    this.displaySerialManager.onConnectionChange = (connected) => {
      this.isDisplayConnected = connected;
      this.addDebugMessage(
        connected ? "Display connected" : "Display disconnected"
      );
    };
  ...
  methods: {
    ...
    async attemptAutoConnect() {
      try {
        ...
        if (ports.length > 0) {
          ...
          const port = ports[0];
          this.addDebugMessage(
            "Attempting auto-connection to previously used port..."
          );
    ...
    async toggleDisplayConnection() {
      if (this.isDisplayConnected) {
        await this.displaySerialManager.disconnect();
      } else {
        await this.displaySerialManager.connect();
      }
    },

    async setDisplayBrightness(level) {
      try {
        await this.displaySerialManager.setBrightness(level);
        this.addDebugMessage(`Display brightness set to level ${level}`);
      } catch (error) {
        this.addDebugMessage(`Display brightness error: ${error.message}`);
      }
    },

    async clearDisplay() {
      try {
        await this.displaySerialManager.clearDisplay();
        this.addDebugMessage("Display cleared");
      } catch (error) {
        this.addDebugMessage(`Display clear error: ${error.message}`);
      }
    },

    async sendTextToDisplay() {
      const zone = parseInt(this.displayText.selectedZone.replace('zone', ''));
      const text = this.displayText[this.displayText.selectedZone];
      const color = this.displayText.color;

      if (!text.trim()) {
        this.addDebugMessage("No text to send to display");
        return;
      }

      try {
        await this.displaySerialManager.sendText(zone, text, color);
        this.addDebugMessage(`Text sent to display zone ${zone}: "${text}" (${color})`);
      } catch (error) {
        this.addDebugMessage(`Display text error: ${error.message}`);
      }
    },
    ...
    startCoursewalks() {
      if (this.coursewalks.running) return;

      this.coursewalks.running = true;
      this.coursewalks.currentPhase = "Starting...";
      this.coursewalks.progressPercent = 0;

      const totalTime = (this.coursewalks.duration * 60) + this.coursewalks.wait;
      let elapsed = 0;

      this.coursewalks.timer = setInterval(() => {
        elapsed++;
        this.coursewalks.progressPercent = (elapsed / totalTime) * 100;

        if (elapsed <= this.coursewalks.wait) {
          this.coursewalks.currentPhase = "Waiting...";
          this.coursewalks.timeRemaining = `${this.coursewalks.wait - elapsed}s`;
        } else {
          const walkTime = elapsed - this.coursewalks.wait;
          const walkTimeRemaining = (this.coursewalks.duration * 60) - walkTime;
          this.coursewalks.currentPhase = `${this.coursewalks.version} Active`;

          if (walkTimeRemaining > 0) {
            const minutes = Math.floor(walkTimeRemaining / 60);
            const seconds = walkTimeRemaining % 60;
            this.coursewalks.timeRemaining = `${minutes}:${seconds.toString().padStart(2, '0')}`;
          } else {
            this.coursewalks.currentPhase = "Completed";
            this.coursewalks.timeRemaining = "0:00";
            this.stopCoursewalks();
          }
        }
      }, 1000);

      this.addDebugMessage(`Coursewalks started: ${this.coursewalks.version}, ${this.coursewalks.duration}min, ${this.coursewalks.wait}s wait`);
    },

    stopCoursewalks() {
      if (this.coursewalks.timer) {
        clearInterval(this.coursewalks.timer);
        this.coursewalks.timer = null;
      }
      this.coursewalks.running = false;
      this.coursewalks.currentPhase = "";
      this.coursewalks.timeRemaining = "";
      this.coursewalks.progressPercent = 0;
      this.addDebugMessage("Coursewalks stopped");
    },

    // ===== ONLINA DATA INTEGRATOR METHODS =====

    startOnlinaPolling() {
      if (this.onlinaData.isPolling || !this.onlinaData.url) return;

      this.onlinaData.isPolling = true;
      this.onlinaData.error = null;

      const poll = async () => {
        try {
          // Use Netlify proxy for CORS
          const proxyUrl = `/.netlify/functions/proxy?url=${encodeURIComponent(this.onlinaData.url)}`;
          ...
          this.onlinaData.currentData = {
            dorsal: data.dorsal || data.number || '',
            handler: data.handler || data.competitor || '',
            dog: data.dog || data.dogName || '',
            country: data.country || data.nation || '',
            faults: data.faults || 0,
            refusals: data.refusals || 0,
            disqualified: data.disqualified || data.eliminated || false
          };
          this.onlinaData.lastUpdated = new Date().toLocaleTimeString();
          this.onlinaData.error = null;

        } catch (error) {
          this.onlinaData.error = `Polling error: ${error.message}`;
          this.addDebugMessage(`ONLINA polling error: ${error.message}`);
        }
      };

      // Poll immediately, then every 2 seconds
      poll();
      this.onlinaData.pollTimer = setInterval(poll, 2000);
      this.addDebugMessage(`Started ONLINA polling: ${this.onlinaData.url}`);
    },

    stopOnlinaPolling() {
      if (this.onlinaData.pollTimer) {
        clearInterval(this.onlinaData.pollTimer);
        this.onlinaData.pollTimer = null;
      }
      this.onlinaData.isPolling = false;
      this.addDebugMessage("ONLINA polling stopped");
    },
  },
}).mount("#app");
